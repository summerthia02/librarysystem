    ...
    def __init__(self, parent, controller):
        ...
        def callDelete(inputCur, inputConn):
            ...
            if membershipTuple:
                ...
                # check for loan // if member is loan ->
                flagonloan = False
                cur.execute('SELECT memberID FROM Loans')
                allmemberloans = cur.fetchall()
                for memberloans in allmemberloans:
                    if memberloans[0] == memberId.get():
                        flagonloan = True
                if flagonloan == True:
                    return errorFrame(self.controller, "Member has loans;\nMissing or Incomplete fields.", "Delete")
                
                # reservation
                flagreserves = False
                cur.execute('SELECT memberID FROM Reserves')
                allmembereserves = cur.fetchall()
                for memberreserves in allmembereserves:
                    if memberreserves[0] == memberId.get():
                        flagreserves = True
                if flagreserves == True:
                    return errorFrame(self.controller, "Member has Reservations;", "Delete")
                #fine
                flagfines = False
                cur.execute('SELECT memberID FROM Fine')
                alllmemberfines = cur.fetchall()
                for memberfines in alllmemberfines:
                    if memberfines[0] == memberId.get():
                        flagfines = True
                if flagfines == True:
                    return errorFrame(self.controller, "Member has Fines;\.", "Delete")
